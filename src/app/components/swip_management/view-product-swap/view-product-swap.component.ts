...
@Component({
  selector: 'app-view-product-swap',
  templateUrl: './view-product-swap.component.html',
  styleUrls: ['./view-product-swap.component.css'],
})
export class ViewProductSwapComponent implements OnInit {
  produit: any;
  id_product: any;
  id_user: any;
  user: any;
  ...
  ngOnInit(): void {
    ...
    this.getUser(this.produit.user);
  ...
  getProduct() {
    ...
    this.productService.getProductById(this.id_product).subscribe((data) => {
  ...
  Addavispro() {
    if (this.id_user == null) {
      ...
      if (this.ok == true) {
        ...
        this.avisproduitService
          .AddAvisProduct(this.avispro)
          .subscribe((data) => {
            this.ngOnInit();
          });
      } else {
  ...
  getReview(id: any) {
    ...
    this.avisproduitService.getReviewProduct(id).subscribe((data) => {
    ...
    this.avisproduitService.getNumber(id).subscribe((data) => {
  ...
  getPanier(panier: any) {
    ...
    this.panierService.getProductByID(panier).subscribe((result) => {
  ...
  SaveInPanier(produit: any) {
    produit.user = this.id_user;
    this.panierService.AddPanier(produit).subscribe((data) => {
  ...
  DeleteOne() {
    ...
    this.panierService
      .deleteProductFromBasket(this.produit)
      .subscribe((data) => {
        this.ngOnInit();
      });
  }
  ...
  addCart(produit: any) {
    ...
    if (produit.quantity <= type) {
      ...
      console.log('benda5loha fi bd ', produit);
