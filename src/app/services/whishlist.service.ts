...
export class WhishlistService {
  ...
  GetNumberWish(id: any) {
    return this.httpClient.get<{ number: any }>(
      `${this.wishlistUrl}/api/whish/number/${id}`
    );
  }

  deblicated() {
    return this.httpClient.get<{ wish: any }>(
      `${this.wishlistUrl}/api/whish/deblicate`
    );
  }

  GetInformation(wish: any) {
    return this.httpClient.get<{ wish: any }>(
      `${this.wishlistUrl}/api/whish/information/${wish}`
    );
  }
}
