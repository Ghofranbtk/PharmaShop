...
router.post("/add", (req, res) => {
  ...
  Category.findOne({ name: req.body.name_categorie }).then((data) => {
    if (data) {
      res.status(200).json({
...
router.get("/number/:category", (req, res) => {
  console.log(
    "here in get number of products of this category :",
    req.params.category
  );
  Product.count({ categorie: req.params.category }).then((data) => {
...
router.get("/payment/:category", (req, res) => {
  console.log(
    "here in get number of payment of this category :",
    req.params.category
  );
  Product.find({ categorie: req.params.category }).then((data) => {
    var x = 0;
