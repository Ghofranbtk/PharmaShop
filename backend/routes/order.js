...
router.post("/add", (req, res) => {
  ...
  for (let i = 0; i < order.products.length; i++) {
    Product.findOne({ name: order.products[i].name }).then((data) => {
      //data.nbr_payment = data.nbr_payment + 1 ;
      //   Product.updateOne({ name: data.name }, data).then((result) => {
      //     console.log("Updated !");
      //   });
    });
  }
...
router.get("/user/:id", (req, res) => {
  console.log("here in get order  by id ", req.params.id);
  Order.find({ user: req.params.id }).then((data) => {
    if (data) {
      res.status(200).json({
        order: data,
      });
...
router.delete("/validate/:id", (req, res) => {
  ...
  Order.findOne({ _id: req.params.id }).then((data) => {
    data.status = true;
    Order.updateOne({ _id: req.params.id }, data).then((result) => {
      res.status(200).json({
        message: "Updated !",
      });
    });
...
router.get("/validated/all", (req, res) => {
  ...
  Order.find({ status: true }).then((data) => {
    res.status(200).json({
      orders: data,
    });
  });
...
router.get("/no_validate/all", (req, res) => {
  ...
  Order.find({ status: false }).then((data) => {
    res.status(200).json({
      orders: data,
    });
  });
