...
router.get("/panier/:id", (req, res) => {
  ...
  Panier.find({ user: req.params.id }).then((result) => {
    if (result) {
      res.status(200).json({
        panier: result,
      });
...
router.post("/cart/add", (req, res) => {
  ...
  Product.findOne({ name: req.body.name, user: req.body.user }).then((data) => {
    if (data) {
...
//---------------------Add product in cart ---------------------
router.post("/cart/add_swap", (req, res) => {
  console.log("Here into Post cart ", req.body);
  // if (req.body.solde == true){
  // const pricee = req.body.new_price ;
  // console.log('priiicee',pricee);
  ...
  Swap.findOne({ name: req.body.name, user: req.body.user }).then((data) => {
    if (data) {
      Panier.findOne({ user: req.body.user, name: req.body.name }).then(
        (result) => {
          if (result) {
...
router.post("/product/add-swap", (req, res) => {
  console.log("Here into Post Panier ", req.body);
  // if(req.body.solde == true){
  ...
  const panier = new Panier({
    ...
    stock: req.body.stock,
    user: req.body.user,
    swap_user: req.body.swap_user,
  });
  Swap.findOne({ name: req.body.name }).then((result) => {
    if (result) {
...
router.get("/panier/prices/:id", (req, res) => {
  ...
  let p = 0;
  let n = 0;
  Panier.find().then((result) => {
    Panier.count().then((number) => {
...
router.post("/products", (req, res) => {
  ...
  Panier.findOne({ name: req.body.name }).then((data) => {
    if (data) {
      res.status(200).json({
        pro: data,
      });
