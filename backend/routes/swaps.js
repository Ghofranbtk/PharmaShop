...
const Product = require("../models/swaps");
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const imgName = name + "-" + Date.now() + "-RoboShop-" + "." + extension;
    cb(null, imgName);
  },
});
...
router.post(
  "/upload",
  multer({ storage: storage }).single("imgg"),
  (req, res) => {
...
router.get("/instock", (req, res) => {
  ...
  Product.find({ stock: true }).then((result) => {
    if (result) {
      Product.count().then((nbr) => {
        res.status(200).json({
          products: result,
...
router.get("/category/:id", (req, res) => {
  ...
  Product.find({ categorie: req.params.id }).then((result) => {
    if (result) {
      var data = new Array();
      for (let i = 0; i < 5; i++) {
        data.push(result[i]);
...
router.get("/products/user/:id", (req, res) => {
  console.log("Here In Get Products By User :", req.params.id);
  Product.find({ user: req.params.id }).then((result) => {
    if (result) {
      var data = new Array();
      for (let i = 0; i < 3; i++) {
        data.push(result[i]);
...
router.delete("/products/:id", (req, res) => {
  ...
  Product.deleteOne({ _id: req.params.id }).then((resulft) => {
    if (resulft) {
      res.status(200).json({
        message: "Deleted Product with success ",
      });
...
router.post("/delete/quantity", (req, res) => {
  ...
  Product.findOne({ name: req.body.name }).then((data) => {
    ...
    Product.updateOne({ name: req.body.name }, data).then((result) => {
      res.status(200).json({
